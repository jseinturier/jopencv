...
SET PYTHON3_DIR=D:\programs\python\Python38
SET PYTHON3_LIB=python38.lib
rem SET PYTHON3_CMAKE=-DBUILD_opencv_python3:BOOL=ON -DPYTHON3_EXECUTABLE=%PYTHON3_DIR%/python.exe -DPYTHON3_INCLUDE_DIR=%PYTHON3_DIR%/include -DPYTHON3_LIBRARIES=%PYTHON3_DIR%/Lib -DPYTHON3_LIBRARY=%PYTHON3_DIR%/libs/%PYTHON3_LIB% -DPYTHON3_PACKAGE_PATH=%PYTHON3_DIR%/Lib/site-packages -DPYTHON3_NUMPY_INCLUDE_DIRS=%PYTHON3_DIR%/Lib/site-packages/numpy/core/include

SET PYTHON2_DIR=D:\programs\python\python27
SET PYTHON2_LIB=python27.lib
rem SET PYTHON2_CMAKE=-DBUILD_opencv_python2:BOOL=ON -DBUILD_opencv_python_bindings_generator:BOOL=ON -DPYTHON2_EXECUTABLE=%PYTHON2_DIR%/python.exe -DPYTHON2_INCLUDE_DIR=%PYTHON2_DIR%/include -DPYTHON2_LIBRARIES=%PYTHON2_DIR%/Lib -DPYTHON2_LIBRARY=%PYTHON2_DIR%/libs/%PYTHON2_LIB% -DPYTHON2_PACKAGE_PATH=%PYTHON2_DIR%/Lib/site-packages -DPYTHON2_NUMPY_INCLUDE_DIRS=%PYTHON3_DIR%/Lib/site-packages/numpy/core/include
...
@SET CMAKE_OPTIONS=%PYTHON3_CMAKE% %PYTHON2_CMAKE% %JAVA_CMAKE% -DBUILD_PERF_TESTS:BOOL=OFF -DBUILD_TESTS:BOOL=OFF -DOPENCV_ENABLE_NONFREE:BOOL=ON -DBUILD_DOCS:BOOL=OFF -DBUILD_EXAMPLES:BOOL=OFF -DBUILD_SHARED_LIBS:BOOL=OFF
...
cmake -G%CMAKE_GENERATOR% -T%CMAKE_GENERATOR_TOOLSET% %CMAKE_OPTIONS% %IPP_INTEGRATION% %TBB_INTEGRATION% %CUDA_INTEGRATION% -DOPENCV_EXTRA_MODULES_PATH=%local_repository%\opencv_contrib\modules -DCMAKE_INSTALL_PREFIX=..\..\install\opencv %local_repository%\opencv > %logfile% 2>&1
